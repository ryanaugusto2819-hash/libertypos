...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "read") {
      ...
      const validStatusEnv = ["não enviado", "enviado", "a retirar", "retirado", "a enviar", "saldo de expedição insuficiente", "em separação", "entregue", "em devolução", "devolvido", "sem estoque", "aguardando retirada"];
